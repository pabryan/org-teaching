...
def plotly_export_html(fig, filename, width, height):
    ...
    <iframe class="plotly hide-no-js" allowfullscreen="" frameborder="0" width="{width}" height="{height}" scrolling="no" allowtransparency="true" src="{filename}"></iframe>
    ,#+END_export
    """
...
def plotly_export(fig, plotname, backend, **kwargs):
    ...
    if backend == "html":
        ret = plotly_export_html(fig, f"/{relhtmldir}/{plotname}", width, height)
    elif backend == "latex":
        ret = plotly_export_latex(fig, f"{texdir}/{plotname}.png")
    else:
        ret = plotly_export_static(fig, f"{texdir}/{plotname}.png")
